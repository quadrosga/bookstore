...
class TestProductViewSet(APITestCase):
    ...
    def test_get_single_product(self):
        ...
        response = self.client.get(
            reverse(
                "product-detail", kwargs={"version": "v1", "pk": self.product.id}
            )  # gets product passing id, dinamically creates url for endpoint product-detail
        )

        self.assertEqual(
            response.status_code, status.HTTP_200_OK
        )  # check that requisition worked
        product_data = json.loads(
            response.content
        )  # convert json string from response into structured dict

        self.assertEqual(
            product_data["title"], self.product.title
        )  # check if data returned forom api match object created by setup
    ...
    def test_delete_product(self):
        ...
        response = self.client.delete(
            reverse(
                "product-detail", kwargs={"version": "v1", "pk": self.product.id}
            )  # delete product, finds it by id
        )

        self.assertEqual(
            response.status_code, status.HTTP_204_NO_CONTENT
        )  # checks that content no longer exists
        self.assertFalse(
            Product.objects.filter(id=self.product.id).exists()
        )  # fails if product is still in the db
